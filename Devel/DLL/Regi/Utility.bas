...
Public Function ControlsSet(rs As ADODB.Recordset, lstControls As Access.Controls) As Boolean
    ...
    For Each oControl In lstControls    'Végigmegyünk az összes kontrollon, ami a form-on van
...
Public Function ControlsGet(ByRef lstFields As Collection, lstControls As Access.Controls) As Boolean
    ...
    Dim oControl As Access.Control  'segédváltozó ciklushoz
    Dim sFieldName As String    'Aktuális mezõ neve
    
    For Each oControl In lstControls    'Végigmegyünk a form kontrolljain.
        sFieldName = Mid(oControl.Name, 4)  'A kontroll neve tartalmazza a hozzá kapcsolt mezõ nevét.
        
        Select Case Left(oControl.Name, 3)  'A kontroll nevének prefixuma alapján máshogy kell kiolvasni.
            Case "txt"  'Ha textbox
                If oControl.Value = "" Then  'Ha üres a textbox,
                    lstFields.Add Null, sFieldName  '- kerül tárolásra.
                Else    'Ha nem üres,
                    lstFields.Add Trim(oControl.Value), sFieldName  'a beírt szöveg tárolandó.
                End If
