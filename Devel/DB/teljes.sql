...
CREATE DATABASE [SZETAV]  ON (NAME = N'SZETAV_Data', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL\Data\SZETAV_Data.MDF' , SIZE = 73, FILEGROWTH = 10%) LOG ON (NAME = N'SZETAV_Log', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL\Data\SZETAV_Log.LDF' , SIZE = 2, FILEGROWTH = 10%)
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[TAB_vissz]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [dbo].[TAB_vissz]
GO
...
GRANT  SELECT ,  UPDATE ,  INSERT ,  DELETE  ON [dbo].[TAB_vissz]  TO [public]
GO
...
INSERT INTO TAB_vissz
SELECT
	MUNKALAP.ID,--SORSZ,
	KIALLDAT,
	RTRIM(HIBLEIR) AS HIBLEIR,
	RTRIM(MUVLEIR) AS MUVLEIR,
	RTRIM(NEV) AS DOLGOZO,
	MUNDAT,
	RTRIM(MEGNEV) AS MEGNEV,
	RTRIM(TELEPHSZ) AS TELEPHSZ
FROM
